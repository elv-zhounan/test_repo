...
def non_max_suppression(bboxes, iou_threshold, threshold, box_format="corners"):
    ...
    # each box [best_cls, score, x, y, w, h]
    # we are only interested in those who has a conf_score higher than the threshold
...
def get_evaluation_bboxes(
    ...
    for batch_idx, (x, labels) in enumerate(tqdm(loader)):
        ...
        for idx in range(batch_size):
            ...
            # here,  we add a train_idx in the front of each bbox
            # each box in origin_bboxes with the format of [best_class, confidence_score, x, y, w, h]
...
def check_(model, loader, threshold):
    model.eval()
    ...
    print(f"No obj recall is: {(correct_noobj/(tot_noobj+1e-16))*100:2f}%")
    print(f"Obj recall is: {(correct_obj/(tot_obj+1e-16))*100:2f}%")
    model.train()
